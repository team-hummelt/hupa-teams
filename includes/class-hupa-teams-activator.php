...
class Hupa_Teams_Activator
{
    ...
    {
           self::hupa_register_team_members();
           self::hupa_register_team_members_taxonomies();
           flush_rewrite_rules();
    }
     ...
     * @since    1.0.0
     */
    public static function hupa_register_team_members(): void
    {
        register_post_type(
            'team_members',
            array(
                'labels' => array(
                    'name' => __('Team Members', 'hupa-teams'),
                ...
                'menu_icon' => self::get_svg_icons('personen'),
                'menu_position' => 104,
                'supports' => array(
                    'title', 'excerpt', 'page-attributes', 'author', 'editor', 'thumbnail','custom-fields'
                ),
                'taxonomies' => array('team_members_category'),
            )
        );
    }
     ...
     * @since    1.0.0
     */
    public static function hupa_register_team_members_taxonomies(): void
    {
        $labels = array(
        ...
        $args = array(
            ...
            'rewrite' => array('slug' => 'team_members_category'),
            'show_admin_column' => true
        );
        register_taxonomy('team_members_category', array('team_members'), $args);

        if (!term_exists('Members General', 'team_members_category')) {
            wp_insert_term(
                'Members General',
                'team_members_category',
                array(
                    'description' => __('Standard category for Team Members posts', 'hupa-teams'),
                    'slug' => 'team-members-posts'
                )
            );
        }
    }
