 ...
 * @Since 1.0.0
 */
class Render_Callback_Templates
{
     ...
     */
    public function __construct(string $plugin_name, string $version, Hupa_Teams $main, Environment $twig)
    {
    ...
    public function render_callback_template($attributes)
    {
        ...
        $args = [
            ...
            'tax_query' => [
                [
                    'taxonomy' => 'team_members_category',
                    'field' => 'term_id',
        ...
        foreach ($queryObj->posts as $tmp) {
            $imgMeta = get_post_meta($tmp->ID, '_member_cover_image_meta', true);
            $showMember = get_post_meta($tmp->ID, '_team_member_show', true);
            if (!(int)$showMember) {
                $count--;
                continue;
            }
            ...
            $data_item = [
                ...
                'img_url' => $imgUrl,
                'content' => $content,
                'name' => get_post_meta($tmp->ID, '_team_member_name', true),
                'subName' => get_post_meta($tmp->ID, '_team_member_subtitle', true),
                'show' => true,
                'scrollAktiv' => (bool) get_post_meta($tmp->ID, '_team_member_scroll_top', true),
                'scrollOffset' =>  abs(get_post_meta($tmp->ID, '_team_member_scroll_offset', true))
            ];
            $dataArr[] = $data_item;
        }
        if( $plusBox != 0) {
